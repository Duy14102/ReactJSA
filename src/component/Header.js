...
function Header({ type }) {
    ...
    return (
        <>
            <nav style={{ backgroundColor: type ? "#0F172B" : "transparent", position: type ? "static" : "fixed", width: "100%", zIndex: type ? null : 5000 }} className={"navbar navbar-expand-lg navbar-dark px-4 px-lg-5 py-3 py-lg-0"}>
                ...
                <div className="d-flex fatherSvgCart1">
                    <NavLink reloadDocument to="/Cart" className="nav-item nav-link responFormSearch">
                        ...
                        <span className='badge' id='lblCartCount'> {JSON.parse(localStorage.getItem('cart')) ? JSON.parse(localStorage.getItem('cart')).length : 0} </span>
                    </NavLink>
                                    ...
                                    return (
                                        <div key={i._id} className="Move1">
                                            ...
                                            <div className="dropdown-menu m-0 text-center Move3">
                                                <NavLink reloadDocument to={decode.userRole === 1 ? `/UserPanel/${i._id}` : decode.userRole === 2 ? "/EmployeePanel" : decode.userRole === 2.5 ? "/ChefPanel" : decode.userRole === 3 ? "/ManagerPanel" : decode.userRole === 4 ? "/AdminPanel" : null} className="dropdown-item">{decode.userRole === 1 ? "Account" : decode.userRole === 2 ? "Employee Panel" : decode.userRole === 2.5 ? "Chef Panel" : decode.userRole === 3 ? "Manager Panel" : decode.userRole === 4 ? "Admin Panel" : null}</NavLink>

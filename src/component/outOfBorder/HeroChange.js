...
function HeroChange({ data, decode }) {
    ...
    useEffect(() => {
        ...
        socketRef.current.on('ChangeHeroImageSuccess', dataGot => {
            if (dataGot.title === "Hero" && dataGot.mag === decode.userId) {
                Success()
            }
        })
    ...
    const changeImage = (e, name) => {
        e.preventDefault()
        const data = { title: "Hero", name: name, image: image, mag: decode.userId }
        socketRef.current.emit('ChangeHeroImageSocket', data)
    }

    const changeWordUp = (e) => {
        e.preventDefault()
        const data = { title: "Hero", wordup: wordup, mag: decode.userId }
        socketRef.current.emit('ChangeWordUpSocket', data)
    }

    const changeWordMiddle = (e) => {
        e.preventDefault()
        const data = { title: "Hero", wordmiddle: wordmiddle, mag: decode.userId }
        socketRef.current.emit('ChangeWordMiddleSocket', data)
    }

    const changeWordDown = (e) => {
        e.preventDefault()
        const data = { title: "Hero", worddown: worddown, mag: decode.userId }
                            ...
                            <div className="joincoon py-5">
                                <div className="sonOfcoon">
                                    ...
                                    {checkword1 ? (
                                        ...
                                        <div className="wordTomcat">
                                            <textarea style={{ pointerEvents: "none" }} className="textDeny junlen" id="blah10" defaultValue={HTMLReactParser(i.word.up).props?.children ? HTMLReactParser(i.word.up).props.children : HTMLReactParser(`${i.word.up}`)} />
                                ...
                                <div className="sonOfcoon">
                                    ...
                                    {checkword2 ? (
                                        ...
                                        <div className="wordTomcat" >
                                            <textarea style={{ pointerEvents: "none" }} className="textDeny junlen" id="blah9" defaultValue={HTMLReactParser(i.word.middle).props?.children ? HTMLReactParser(i.word.middle).props.children : HTMLReactParser(`${i.word.middle}`)} />
                                ...
                                <div className="sonOfcoon">
                                    ...
                                    {checkword3 ? (
                                        ...
                                        <div className="wordTomcat">
                                            <textarea style={{ pointerEvents: "none" }} className="textDeny junlen" id="blah8" defaultValue={HTMLReactParser(i.word.down).props?.children ? HTMLReactParser(i.word.down).props.children : HTMLReactParser(`${i.word.down}`)} />

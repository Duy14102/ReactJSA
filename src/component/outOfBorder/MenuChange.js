...
function MenuChange({ data, decode }) {
    ...
    const changeImage = (e, name) => {
        e.preventDefault()
        const data = { title: "Menu", name: name, image: image, mag: decode.userId }
        socketRef.current.emit('ChangeHeroImageSocket', data)
    }

    const changeWordUp = (e) => {
        e.preventDefault()
        const data = { title: "Menu", wordup: wordup, mag: decode.userId }
        socketRef.current.emit('ChangeWordUpSocket', data)
    }

    const changeWordMiddle = (e) => {
        e.preventDefault()
        const data = { title: "Menu", wordmiddle: wordmiddle, mag: decode.userId }
        socketRef.current.emit('ChangeWordMiddleSocket', data)
    }

    const changeWordDown = (e) => {
        e.preventDefault()
        const data = { title: "Menu", worddown: worddown, mag: decode.userId }

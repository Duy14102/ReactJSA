...
import NotFound from '../outOfBorder/NotFound';
import Swal from 'sweetalert2';
import { NavLink } from 'react-router-dom';

function LoginBase({ type }) {
    const cookies = new Cookies();
    const token = cookies.get("TOKEN");
    const [loginState, setLoginState] = useReducer((prev, next) => ({
        ...prev, ...next
    }), {
        email: "",
        password: "",
                ...
                <div className="wrap-login100">
                    <form onSubmit={(e) => handleSubmit(e)} className="login100-form validate-form">
                        ...
                        <div className="wrap-input100 validate-input" data-validate="Valid email is: a@b.c">
                            <input className="input100" type="email" name="email" value={loginState.email} onChange={(e) => setLoginState({ email: e.target.value })} required />
                            <span className="focus-input100" data-placeholder="Email"></span>
                        </div>

                        <div className="wrap-input100 validate-input" data-validate="Enter password">
                            {loginState.showPass ? (
                                <span onClick={() => setLoginState({ showPass: false })} className="btn-show-pass">
                                ...
                                <span onClick={() => setLoginState({ showPass: true })} className="btn-show-pass">
                            ...
                            <input className="input100 showpassInp" type="password" name="password" value={loginState.password} onChange={(e) => setLoginState({ password: e.target.value })} required />

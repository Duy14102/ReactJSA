...
function LoginSite() {
    ...
    const handleSubmit = (e) => {
        ...
        if (registerState.password !== registerState.confirm) {
            return false
        } if (!checkUpperCase.test(registerState.password)) {
            return false
        } if (!checkDigit.test(registerState.password)) {
            return false
        } if (!checkPhone.test(registerState.phone)) {
            setRegisterState({ alertCheck: true })
            return false
        }
        const data = { email: registerState.email, password: registerState.password, fullname: registerState.fullname, phone: registerState.phone }
        localStorage.setItem("CheckEmail", registerState.email)
        socketRef.current.emit('RegisterSocket', data)
    }

    useEffect(() => {
        if (registerState.showPass) {
            $('.showpassInp1').attr('type', 'text')
        } else {
            $('.showpassInp1').attr('type', 'password')
        }
    }, [registerState.showPass])

    useEffect(() => {
        if (registerState.showPass2) {
            $('.showpassInp2').attr('type', 'text')
        } else {
            $('.showpassInp2').attr('type', 'password')
        }
    }, [registerState.showPass2])
                    ...
                    <div className="wrap-login100">
                        <form onSubmit={(e) => handleSubmit(e)} className="login100-form validate-form">
                            ...
                            <div className="wrap-input100 validate-input" data-validate="Valid email is: a@b.c">
                                <input className="input100" type="email" name="email" value={registerState.email} onChange={(e) => setRegisterState({ email: e.target.value })} required />
                            ...
                            <div style={{ marginBottom: 50 + "px", position: "relative" }}>
                                <div className="wrap-input100 validate-input m-0" data-validate="Enter password">
                                    {registerState.showPass ? (
                                        <span onClick={() => setRegisterState({ showPass: false })} className="btn-show-pass">
                                        ...
                                        <span onClick={() => setRegisterState({ showPass: true })} className="btn-show-pass">
                                    ...
                                    <input className="input100 showpassInp1" type="password" name="password" value={registerState.password} onChange={(e) => setRegisterState({ password: e.target.value })} required />
                                    <span className="focus-input100" data-placeholder="Password"></span>
                                </div>
                                {registerState.password !== "" ? (
                                    <div className='appearCheckPass'>
                                        {checkUpperCase.test(registerState.password) ? null : (
                                            <p>❌ Password must have at least 1 uppercase letter</p>
                                        )}
                                        {checkDigit.test(registerState.password) ? null : (
                            ...
                            <div style={{ marginBottom: 50 + "px", position: "relative" }}>
                                <div className="wrap-input100 validate-input m-0" data-validate="Repeat password">
                                    {registerState.showPass2 ? (
                                        <span onClick={() => setRegisterState({ showPass2: true })} className="btn-show-pass">
                                            <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 640 512"><path d="M38.8 5.1C28.4-3.1 13.3-1.2 5.1 9.2S-1.2 34.7 9.2 42.9l592 464c10.4 8.2 25.5 6.3 33.7-4.1s6.3-25.5-4.1-33.7L525.6 386.7c39.6-40.6 66.4-86.1 79.9-118.4c3.3-7.9 3.3-16.7 0-24.6c-14.9-35.7-46.2-87.7-93-131.1C465.5 68.8 400.8 32 320 32c-68.2 0-125 26.3-169.3 60.8L38.8 5.1zM223.1 149.5C248.6 126.2 282.7 112 320 112c79.5 0 144 64.5 144 144c0 24.9-6.3 48.3-17.4 68.7L408 294.5c8.4-19.3 10.6-41.4 4.8-63.3c-11.1-41.5-47.8-69.4-88.6-71.1c-5.8-.2-9.2 6.1-7.4 11.7c2.1 6.4 3.3 13.2 3.3 20.3c0 10.2-2.4 19.8-6.6 28.3l-90.3-70.8zM373 389.9c-16.4 6.5-34.3 10.1-53 10.1c-79.5 0-144-64.5-144-144c0-6.9 .5-13.6 1.4-20.2L83.1 161.5C60.3 191.2 44 220.8 34.5 243.7c-3.3 7.9-3.3 16.7 0 24.6c14.9 35.7 46.2 87.7 93 131.1C174.5 443.2 239.2 480 320 480c47.8 0 89.9-12.9 126.2-32.5L373 389.9z" /></svg>
                                        </span>
                                    ) : (
                                        <span onClick={() => setRegisterState({ showPass2: true })} className="btn-show-pass">
                                    ...
                                    <input className="input100 showpassInp2" type="password" name="password" value={registerState.confirm} onChange={(e) => setRegisterState({ confirm: e.target.value })} required />
                            ...
                            <div className="wrap-input100 validate-input" data-validate="Enter Fullname">
                                <input className="input100" type="text" name="fullname" value={registerState.fullname} onChange={(e) => setRegisterState({ fullname: e.target.value })} required />
                            ...
                            <div style={{ marginBottom: 50 + "px", position: "relative" }}>
                                <div className="wrap-input100 validate-input m-0" data-validate="Enter Phone Number">
                                    <input className="input100" type="number" name="phone" value={registerState.phone} onChange={(e) => setRegisterState({ phone: e.target.value })} required />
                                    <span className="focus-input100" data-placeholder="Enter Phone Number"></span>
                                </div>
                                {registerState.alertCheck ? (
                                    <div className='appearCheckPass'>
